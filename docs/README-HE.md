...
## תמיכה במולטי-פלטפורמה

כאשר הפרויקט כולל מספר אפליקציות (למשל: אפליקציית לקוחות + פאנל אדמין + פורטל ספקים), המערכת תומכת ב-**מולטי-פלטפורמה**.

### איך זה עובד?

**1. זיהוי בשלב Assessment:**
```
שאלה: כמה פלטפורמות יש לפרויקט?
- single (אחת)
- 2-3 apps (כמה אפליקציות)
- many subsystems (הרבה תתי-מערכות)
```

אם יש יותר מפלטפורמה אחת → `status.multiPlatform = true`

**2. הגדרת פלטפורמות בשכבה `platforms/`:**

```toml
# layers/platforms/customer-app.toml
[platform]
id = "customer-app"
...
```
/prototype              → דף בחירת פלטפורמה (Platform Hub)
/prototype/customer     → אפליקציית לקוחות
/prototype/admin        → פאנל אדמין
/prototype/vendor       → פורטל ספקים
```

**4. קומפוננטת Platform Hub:**

דף כניסה שמאפשר לבחור לאיזו פלטפורמה להיכנס - עם כרטיסים לכל פלטפורמה.

### היתרון

- **הפרדה ברורה** - כל פלטפורמה עם routes, roles ו-guards משלה
- **מקור אמת אחד** - כל הפלטפורמות משתמשות באותם entities
- **גמישות** - אפשר להוסיף פלטפורמות בלי לשנות את המנוע

---

## סיכום: מה הייחודיות?

1. **אלגוריתם אדפטיבי** - מתאים את עצמו למורכבות הפרויקט
...
6. **מולטי-פלטפורמה** - תמיכה בפרויקטים עם מספר אפליקציות
