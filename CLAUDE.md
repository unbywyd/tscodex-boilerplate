...
**User Scenario:** User comes with a project idea (e.g., "I want to build a task manager app"). You ask questions, determine complexity, and create specs phase by phase.

## Workflow

1. **User describes idea** → You start Assessment phase
2. **You ask questions** → Based on profile depth (shallow/medium/deep)
3. **You create TOML files** → In `src/spec/layers/` folders
4. **You update status.toml** → After each phase completion
5. **You generate prototype** → React components in `src/prototype/`
